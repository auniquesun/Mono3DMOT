...
def write_results(filename, results, data_type):
    if data_type == 'mot':
        ...
        # save_format = 'frame: {frame}, person_id: {id}, x1: {x1}, y1: {y1}, width: {w}, height: {h}\n\tx: {x_world}, y: {y_world}, z: {z_world}\n'
    ...
    for frame_id, tlwhs, track_ids, locs in results:
        ...
        for tlwh, track_id, loc in zip(tlwhs, track_ids, locs):
            ...
            # line = save_format.format(frame=frame_id, id=track_id, x1=x1, y1=y1, x2=x2, y2=y2, w=w, h=h, x_world=x_world, y_world=y_world, z_world=z_world)
...
if __name__ == '__main__':
    ...
    if opt.val_mot15:
        seqs_str = '''ADL-Rundle-6
                      ...
                      TUD-Stadtmitte
                      Venice-2'''
        data_root = os.path.join(opt.data_dir, '2DMOT2015/train')
    if opt.test_mot15:
        seqs_str = '''ADL-Rundle-1
                      ...
                      TUD-Crossing
                      Venice-1'''
        data_root = os.path.join(opt.data_dir, '2DMOT2015/test')
    if opt.val_mot16:
        seqs_str = '''MOT16-02
                      MOT16-04
                      MOT16-05
