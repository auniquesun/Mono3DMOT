...
def plot_tracking(image, tlwhs, obj_ids, locs, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    for i, tlwh_loc in enumerate(zip(tlwhs, locs)):
        """
        tlwh_loc： 是个元组，形式如 (tlwh, loc)
                    tlwh，loc 字如其意
        """
        x1, y1, w, h = tlwh_loc[0]
        intbox = tuple(map(int, (x1, y1, x1 + w, y1 + h)))
        obj_id = int(obj_ids[i])

        id_text = '{}'.format(int(obj_id))
        x, y, z = float("{0:.2f}".format(tlwh_loc[1][0])), float("{0:.2f}".format(tlwh_loc[1][1])), float("{0:.2f}".format(tlwh_loc[1][2]))
        loc_text_x, loc_text_y, loc_text_z = 'x: {}'.format(x), 'y: {}'.format(y), 'z: {}'.format(z)
        ...
        cv2.putText(im, loc_text_x, (intbox[0], intbox[3] - 50), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=text_thickness)
        cv2.putText(im, loc_text_y, (intbox[0], intbox[3] - 35), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=text_thickness)
        cv2.putText(im, loc_text_z, (intbox[0], intbox[3] - 20), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=text_thickness)
    return im
